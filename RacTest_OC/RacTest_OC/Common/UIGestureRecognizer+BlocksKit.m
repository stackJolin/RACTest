...
- (void)bk_handleAction:(UIGestureRecognizer *)recognizer
{
    void (^handler1)(UIGestureRecognizer *sender, UIGestureRecognizerState state, CGPoint location);
	void (^handler)(UIGestureRecognizer *sender, UIGestureRecognizerState state, CGPoint location) = recognizer.bk_handler;
	if (!handler) return;
    id a = recognizer.bk_handler;
...
{
    // 这里使用copy和retain都是一样的，block的retain方法里面也是copy的实现。MRC遗留下来的产物
//	objc_setAssociatedObject(self, BKGestureRecognizerBlockKey, handler, OBJC_ASSOCIATION_COPY_NONATOMIC);
    objc_setAssociatedObject(self, BKGestureRecognizerBlockKey, handler, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    id a = objc_getAssociatedObject(self, BKGestureRecognizerBlockKey);
    int b = 1;
}
