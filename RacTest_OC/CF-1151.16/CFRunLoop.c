...
struct __CFRunLoop {
    ...
    pthread_t _pthread;
    uint32_t _winthread;
    // 一个Mode可以将自己的name 添加到这个集合里面，标明自己是具有commen属性。添加到这个里面的mode，
    // 当_currentMode切换的时候，runloop会自动将_commonModeItems中的<source/observer/timer>同步到具有commen属性的所有的mode里
    // 默认情况下，主线程两个预置mode`kCFRunLoopDefaultMode`和`UITrackingRunLoopMode`都具有commen属性
    CFMutableSetRef _commonModes;
    // 当前运行在commen模式下的`CFRunLoopSource`、`CFRunLoopObserver`、`CFRunLoopTimer`。当切换_currentMode的时候，
    // 会将_commonModeItems中的事件源，同步到_currentMode的`_sources0`、`_srouce1`、`_observers`、`_timers`
